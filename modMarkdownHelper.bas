Attribute VB_Name = "modMarkdownHelper"
' ==========================
' Modulo: modMarkdownHelper
' ==========================
Option Explicit
...
Private Function FilterNonEmpty(arr() As String) As String()
    Dim tmp() As String, i As Long, v As String
    Dim k As Long
    ReDim tmp(0 To 0)
...
Public Sub InsertMarkdownInlineToSelection(ByVal s As String, _
...
' ========= Costruzione payload / Sanificazione =========
...
Public Function BuildMessageFromSelection(ByVal rng As Range) As String
    Dim out As String
...
Public Function TableToMarkdown(ByVal tbl As Table) As String
...
End Function
' Applica il “grigetto” SOLO ai caratteri selezionati
' (esclude il paragrafo finale per evitare propagazione formati)
Public Sub GrayOutRange(ByVal r As Range)
    On Error Resume Next
    Dim work As Range
    Set work = r.Duplicate

    ' Escludi l'ultimo carattere se è un CR (¶)
    If work.Characters.Count > 0 Then
        If AscW(Right$(work.text, 1)) = 13 Then
            work.MoveEnd wdCharacter, -1
        End If
    End If

    ' Selezione vuota dopo esclusione? esci
    If work.Start >= work.End Then Exit Sub

    ' Font grigio (sicuro anche se ci sono tabelle)
    work.Font.Color = RGB(128, 128, 128)
    work.Font.ColorIndex = wdGray50

    ' Niente evidenziazione gialla
    work.HighlightColorIndex = wdNoHighlight

    ' Sfondo tenue opzionale: resta a livello di caratteri (non sul ¶)
